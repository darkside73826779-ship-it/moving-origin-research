...
---
## Current state — updated 2026-08-20 00:29 EDT (L8 spec v2.5 landed — 2nd TASK BUILDER STOP closed; 2,000-rep screening authorization WITHDRAWN pending feasibility gate; CRITIC reviewing v2.5)

**Ball:** CRITIC (reviewing v2.5 with the ARCHITECT's handoff directly). The ARCHITECT landed v2.5 "Close L8 determinism and feasibility gaps" (`5209f33` on `architect/l8-g2g4-remediation`) — §8.10 (8 subsections) closes the TASK BUILDER's second stop. The v2.4 CRITIC handoff marked superseded.

**MATERIAL CHANGE — 2,000-rep screening authorization WITHDRAWN:** the ARCHITECT's feasibility accounting found the full screen is 9.6 million cell repetitions and up to 48 billion bootstrap replicates — materially larger than the reviewed baseline. Rebecca's prior 2,000-rep screening authorization (Entry 88) is WITHDRAWN pending a feasibility benchmark. Commit A must include a deterministic benchmark mode (10 reps × 3 fixed cells × 2 geometries, full 5,000-bootstrap, NO scientific screening). After the benchmark + CRITIC review, Rebecca separately chooses: authorize the full screen, approve an amended/reduced design, or stop. The TASK BUILDER may implement Commit A + the feasibility benchmark ONLY (after CRITIC CLEAR + Rebecca approval) — NOT the screen, NOT Commit B evidence.

**v2.5 §8.10 closes (8 subsections):** (§8.10.1) bootstrap RNG (20 independent streams/attempt) + finite seed-manifest (root digests with ranges); (§8.10.2) exact config values + prohibitions array; (§8.10.3) status vocabulary + 6 run-level apparatus checks; (§8.10.4) calibration algorithm (1,000 pilot reps, bisection, β*=0.3, tol 0.01, 40 iters) + cache manifest; (§8.10.5) 12-case rehearsal fixtures (exact injection mapping); (§8.10.6) transactional JSON+sidecar publication; (§8.10.7) Commit A/Commit B lifecycle (artifacts never claim their own commit SHA); (§8.10.8) feasibility gate (benchmark before screening).
