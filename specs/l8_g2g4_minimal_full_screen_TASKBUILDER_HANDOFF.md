...
5. Per geometry compute `max_primary_false_kill = max over non-apparatus-invalid cells of complete_verdict_false_kill_rate` (null if no eligible cells), `has_apparatus_invalid_cell`, `meets_target = false if has_apparatus_invalid_cell; else (max ≤ 0.10)` (exact `(W,N_w)` geometry), and `on_tested_boundary` (`W∈{50,400}` or `N_w∈{4,64}`). Minimum acceptable battery = first geometry in §3 order with `meets_target`; **STOP / escalate** if none pass OR first passing is on a tested boundary (spec §5.4). Use separate true-effect / null-control denominator fields (`n_valid_true_effect`, `n_valid_null_control`, `n_apparatus_invalid_true_effect/null_control`, `cell_apparatus_invalid`); a geometry with any apparatus-invalid cell cannot qualify.
6. **Include the 7 required deterministic-test categories plus the no-softening threshold subtest (2a)** (spec §5.5) with assertions against pre-computed expected values; no no-op / unconditional-pass tests.
...
- Exactly 20 geometries, 240 ordered cells each, 2,000 attempted per cell per arm; per cell `n_valid_true_effect + n_apparatus_invalid_true_effect + n_instrument_failures_true_effect = n_sims_attempted_true_effect` (and analogously `n_valid_null_control + n_apparatus_invalid_null_control + n_instrument_failures_null_control = n_sims_attempted_null_control`).
- Every cell has `complete_verdict_false_kill_rate` (PRIMARY), `diagnostic_beta_only_any_seed_false_kill_rate`, `diagnostic_five_seed_mean_false_kill_rate`, `null_control_false_pass_rate`, `cell_apparatus_invalid`.
- `complete_verdict_false_kill_rate` uses the valid true-effect denominator (apparatus-invalid excluded); `null_control_false_pass_rate` uses the valid null-control denominator.
- `max_primary_false_kill` per geometry recomputes exactly as the max of its non-apparatus-invalid cells' `complete_verdict_false_kill_rate` (null if no eligible cells); `has_apparatus_invalid_cell` matches; `meets_target = false if has_apparatus_invalid_cell; else max_primary_false_kill ≤ 0.10`; `on_tested_boundary` and `first_passing_on_tested_boundary` recompute per §5.4; `minimum_geometry_satisfying_target` recomputes from the rule (null + STOP if none pass OR first passing on a tested boundary).
- ρ predicate comparison uses `RHO_COMPARE_EPS = 1e-12` (pass iff `ρ_s ≥ 0.8` OR `abs(ρ_s − 0.8) ≤ eps`); the no-softening test asserts `0.8 − 2·eps` fails; tie detection is exact finite binary64 equality.
