...
- `max_primary_false_kill` per geometry recomputes exactly as the max of its cells' `complete_verdict_false_kill_rate` (apparatus-invalid cells excluded); `has_apparatus_invalid_cell` matches; `meets_target = (no apparatus-invalid cells) AND max_primary_false_kill ≤ 0.10`; `on_tested_boundary` and `first_passing_on_tested_boundary` recompute per §5.4; `minimum_geometry_satisfying_target` recomputes from the rule (null + STOP if none pass OR first passing on a tested boundary).
