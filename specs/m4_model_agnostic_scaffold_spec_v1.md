...
1. `describe() -> (manifest_bytes, operation_result_bytes)`;
...
Callable lifecycle state validates against `m4_model_adapter_state_schema_v1.json`. State SHA-256 is calculated over the complete RFC-8785 state object without LF, never over a row name or stored constant. The transition table and exact pre/post objects in `m4_model_callable_fixture_v1.json` are normative: construct and validate the next complete state; hash it; construct the operation result with `prior_state_sha256` equal to the pre-state hash and `post_state_sha256` equal to the post-state hash; then commit the state only after successful result validation. On failure, both digest fields equal the unchanged pre-state hash and `result_state=FAILED`. The fixture fixes complete canonical PASS operation-result artifacts and digests for describe, initialize, reset, step, snapshot, and close. Its `closed` object is the sole normative terminal realization: it retains the snapshotted dependency, episode, manifest, response, request, reset, and snapshot fields; sets `lifecycle_state=CLOSED`, `closed=true`, and `frozen_payload_sha256=null`; and has the fixed canonical digest stored beside it. The operation-result schema retains the mutually exclusive legacy `state_sha256` form solely so previously approved regression fixtures remain schema-valid; every v1.5 callable result MUST use the pre/post pair and MUST omit `state_sha256`. `frozen_payload_sha256` is null until its first successful step, remains unchanged within an episode, becomes null on reset, and is null after close. Prior stored lifecycle row digests are regression expectations only, not a callable state algorithm `[PROPOSED]`.
