...
The exact format/context constructor is `specs/data/m4_context_format_probe_contract_v1.json`; its raw bytes and sidecar are normative `[PROPOSED]`. The call is exactly `tokenizer.apply_chat_template(messages=[{"role":"user","content":"Return one JSON object whose answer field is the string A."}], tokenize=true, add_generation_prompt=true, continue_final_message=false, enable_thinking=false)`. The tokenizer and chat-template bytes are those verified at the rung's immutable revision. The neutral-token insertion pointer, uniqueness checks, encode/decode round trip, target-array construction, stop-array construction, and JSON member order are exactly the algorithms in that artifact; no message, whitespace, template flag, insertion point, stop element, or decoding option may be chosen by the executor `[PROPOSED]`.
...
The preflight result, qualification report, and Phase A transcript reject unknown fields and use RFC-8785 UTF-8 plus one LF for stored JSON; sidecars are lowercase SHA-256, two spaces, basename, LF `[PROPOSED]`. Their nested field names and types are exhaustive in the schemas. Arrays are constructed and stored by `ordinal` ascending; ordinals must be contiguous from zero; a duplicate, gap, omission, reorder, or post-construction sort is `CONFIGURATION_MISMATCH`. A preflight `PASS` has exactly ten `checks` rows in this order: `STACK_INDEX`, `STACK_PLATFORM`, `CHECKPOINT_FILES`, `CANDIDATE_PEER_EQUALITY`, `FP8_METADATA`, `FP8_FINITE_LOGITS`, `FORMAT_JSON`, `CONTEXT_1024`, `CONTEXT_4096`, `CONTEXT_8192`; every row status is `PASS`. The schema rejects omission, duplication, reorder, or `NOT_RUN`. Attempted stages use actual execution order. Failure rows use detection order. The peer projection is constructed by copying exactly `peer_projection_order` from the transcript, in that order, and RFC-8785 hashing the resulting object without LF; `candidate_self_state_vector` is consequently unreachable in the projection. The valid Phase A fixture's required seal is `49bbf38b93bafdaeb6f7e8e38712d88686f15f9ab98034d95c1678036f989c51` `[PROPOSED]`.

Complete canonical fixtures and sidecars are committed for custody and preflight `PASS`, `BLOCKED`, and `FAIL`; qualification `PRIMARY_SELECTED`, `BLOCKED`, and `NO_QUALIFYING_RUNG`; and a schema-valid Phase A record. Fixture zero hashes are literal synthetic fixture values, never wildcard placeholders. The constructor deep-copies the corresponding fixture, replaces only observation fields with measured values, replaces identity hashes with computed hashes, appends array rows in prescribed ordinal order, checks semantic invariants without reordering received rows, validates the schema, RFC-8785 canonicalizes, appends one LF, and publishes the JSON/sidecar pair atomically. Pre-start missing authority/configuration maps to `BLOCKED`; an apparatus or identity check that actually ran and failed maps to `FAIL`; all checks passing maps to `PASS`. Qualification cannot emit a selected status until every required stage passes; exhausted rung failures emit `NO_QUALIFYING_RUNG`; missing battery/Q2 authority emits `BLOCKED` `[PROPOSED]`.

Allowed preflight status is `PASS`, `FAIL`, or `BLOCKED`; qualification status is `PRIMARY_SELECTED`, `FALLBACK_SELECTED`, `NO_QUALIFYING_RUNG`, or `BLOCKED`. A report names every attempted stage, retains every failure, and states the selected serving mode and reason `[PROPOSED]`.

The three expanded context token arrays and their SHA-256 values are intentionally not precomputed in this amendment because doing so requires reading the locally held tokenizer artifact, which is outside the present no-download/no-model-artifact execution authority. This is a fail-closed reconciliation input: before preflight release, an authorized local custody step must materialize the arrays by the committed constructor, record only their sanitized lengths and SHA-256 values in a repository amendment, obtain persistent-CRITIC verification, and receive Rebecca's release. TASK BUILDER may not invent or self-approve them `[PROPOSED]`.
