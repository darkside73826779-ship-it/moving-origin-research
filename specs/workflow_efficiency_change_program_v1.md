...
`tools/workflow_return.py publish` is the guarded mechanical publisher for ordinary role returns `[PROPOSED]`. It requires an exact clean base with staged substantive changes only, creates a substantive result commit, a distinct handoff-only routing-tail commit, and a distinct manifest-only commit/branch without changing the package bytes between them, computes the complete raw Git-blob inventory, validates the common manifest, and runs `workflow_preflight.py --repo-root <exact-target-worktree>` over both the substantive/routing and manifest domains. It installs local refs in one transaction and uses one non-force atomic remote push only after ref-collision, worktree/index stability, completed manual review, classification coverage, and remote-equality checks. Any finding remains blocking until an external classification document binds the exact report digest and finding identity with an explicit permitted disposition and rationale; detector/class tampering, incomplete coverage, force-push, deletion, branch reuse, package-byte drift, or any failed check is STOP. The helper prints the exact base/result/routing/manifest SHAs and never performs review, merge, gate, scoring, seed, science, or role-substitution work `[PROPOSED]`.
...
`python tools/workflow_preflight.py --repo-root <absolute-exact-worktree-root> --base <40-hex> --tip <40-hex> --output <repository-relative-json>` `[PROPOSED]`. The tool may be launched from another checkout only when `--repo-root` names the exact target worktree; its Git objects, path events, scans, and output are then all resolved in that target.
